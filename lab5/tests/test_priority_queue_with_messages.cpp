...
TEST_CASE("PriorityQueueWithMessagesTimestamps test")
{
    ...
    SUBCASE("test is_empty() method")
    {
        ...
        queue.emplace(1, std::chrono::system_clock::now() + std::chrono::minutes(10), 2);
    ...
    {
        queue.push(std::make_tuple(1, std::chrono::system_clock::now() + std::chrono::minutes(10), 2));
        CHECK(queue.size() == 1);
        queue.set_max_amount(1);
        CHECK_THROWS_AS(queue.push(std::make_tuple(1, std::chrono::system_clock::now(), 2)), LimitReachedException);
    }

    SUBCASE("test pop() method")
    {
        auto time_point1 = std::chrono::system_clock::now() + std::chrono::minutes(10);
        auto time_point2 = std::chrono::system_clock::now() + std::chrono::minutes(5);
...
TEST_CASE("Analyzer of PriorityQueueWithMessagesTimestamps")
{
    ...
    SUBCASE("basic check.")
    {
        ...
        CHECK(7 == std::count(std::istreambuf_iterator<char>(inFile), std::istreambuf_iterator<char>(), '\n'));
    }
}
